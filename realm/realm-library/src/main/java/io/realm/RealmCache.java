...
class RealmCache {
    ...
    private static final int TYPE_REALM_REF_COUNTER_INDEX = 0;
    private static final int DYNAMIC_REALM_REF_COUNTER_INDEX = 1;
    // Separated references and counters for typed Realm and dynamic Realm.
    private final RefAndCount[] refAndCounts = new RefAndCount[2];
    ...
    private RealmCache(RealmConfiguration config) {
        ...
        for (int i = 0; i < refAndCounts.length; i++) {
            refAndCounts[i] = new RefAndCount();
        }
    }
    ...
    static synchronized BaseRealm createRealmOrGetFromCache(RealmConfiguration configuration,
        ...
        if (realmClass == Realm.class) {
            refAndCount = cache.refAndCounts[TYPE_REALM_REF_COUNTER_INDEX];
        } else {
            refAndCount = cache.refAndCounts[DYNAMIC_REALM_REF_COUNTER_INDEX];
        }
        ...
        if (refCount == 0) {
            if (realmClass == Realm.class && refAndCount.globalCount == 0) {
                cache.typedColumnIndices = refAndCount.localRealm.get().columnIndices;
            }
    ...
    static synchronized int release(BaseRealm realm) {
        ...
        if (refCount == 0) {
            ...
            int totalRefCount = 0;
            for (RefAndCount it: cache.refAndCounts) {
                totalRefCount += it.globalCount;
            }
    ...
    static int getGlobalRefCount(RealmConfiguration configuration) {
        ...
        int totalRefCount = 0;
        for (RefAndCount refAndCount : cache.refAndCounts) {
